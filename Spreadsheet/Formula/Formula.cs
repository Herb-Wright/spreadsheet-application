...
namespace SpreadsheetUtilities
{
    ...
    public class Formula
    {
        ...
        /// </summary>
        private List<string> formulaIter;
        ...
        public Formula(String formula, Func<string, string> normalize, Func<string, bool> isValid)
        {
            ...
            formString = "";
            foreach (string token in formulaIter)
            {
                formString += token;
            }
        }
            ...
            try
            {
                foreach (string token in formulaIter)
                {
                    if (Regex.IsMatch(token, "[a-zA-Z_0-9]"))
                    {
                        ...
                        EvaluateHelper(operatorStack, valueStack, false);
                    }
                    // check for '+' or '-'
                    else if (Regex.IsMatch(token, "^(\\+|-)$"))
                    {
                        EvaluateHelper(operatorStack, valueStack, true);
        ...
        private static void EvaluateHelper(Stack<string> operatorStack, Stack<double> valueStack, bool isAdd)
        {
                ...
                if ((isAdd && Regex.IsMatch(operatorStack.Peek(), "^(\\+|-)$")) || (!isAdd && Regex.IsMatch(operatorStack.Peek(), "^(\\*|/)$")))
                {
                    switch (operatorStack.Pop())
                    {
                        ...
                        case "/":
                            ...
                            if(value2 == 0) { throw new DivideByZeroException("div by 0"); }
        ...
        {
            return new HashSet<string>(formulaIter.Where<string>(token => Regex.IsMatch(token, "[a-zA-Z_]")));
        }
