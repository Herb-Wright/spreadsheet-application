...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            LinkedList<string> changed = new LinkedList<string>(); // init a linked list for changed cells
            HashSet<string> visited = new HashSet<string>(); // init set for visited cells
            Visit(name, name, visited, changed); // call helper function that recursively visits cells
            return changed; // return the list of cells that where added to changed in visit method.
        }
        ...
        {
            visited.Add(name); // adds the cell to the list of visited cells
            foreach (string n in GetDirectDependents(name)) // loop through the cell's dependents
            {
                if (n.Equals(start)) // check if the cell depends on original cell
                {
                    throw new CircularException(); // throw CircularException error
                }
                else if (!visited.Contains(n)) // check if the cell has not been visited
                {
                    Visit(start, n, visited, changed); // visit the cell (recursive call)
                }
            }
            changed.AddFirst(name); // afterwards, add cell to front of a list of changed cells.
        }
